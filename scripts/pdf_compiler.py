...
class PDFCompiler:
    ...
    def _compile_with_pil(self, image_paths: List[str]) -> bool:
        ...
        try:
            ...
            # Use a generator expression for append_images so PIL iteratively loads
            # each image from disk rather than storing them all in memory at once
            images_generator = (
                Image.open(path).convert('RGB') if Image.open(path).mode != 'RGB' else Image.open(path)
                for path in image_paths[1:]
            )

            first_img.save(self.output_filename, save_all=True, append_images=images_generator)
